...
var ATTACHMENT_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
...
function pruneAttachments(workdir, now = new Date, maxAgeMs = ATTACHMENT_MAX_AGE_MS) {
  ...
  for (const entry of entries) {
    ...
    if (mtime.getTime() > now.getTime() - maxAgeMs)
      continue;
...
function prepareWorkdir(mode, opts) {
  if (mode === "session") {
    ...
    if (worktree === "/") {
      throw new AgyConfigError("worktree", 'session workdirMode refuses the filesystem root "/" \u2014 opencode reports "/" for non-git directories; run inside a git worktree or switch workdirMode to "scratch"');
    }
    let stat;
    try {
...
function parseModelsCache(raw) {
  ...
  if (typeof parsed !== "object" || parsed === null)
    return null;
  const rec = parsed;
  if (rec.version !== MODELS_CACHE_VERSION)
    return null;
  if (typeof rec.fetchedAt !== "string" || Number.isNaN(Date.parse(rec.fetchedAt)))
    return null;
  ...
  for (const entry of rec.models) {
    if (typeof entry !== "object" || entry === null)
      return null;
    const e = entry;
    if (typeof e.id !== "string" || e.id === "" || typeof e.name !== "string")
      return null;
    models.push({ id: e.id, name: e.name });
  }
  return { version: MODELS_CACHE_VERSION, fetchedAt: rec.fetchedAt, models };
}
function cacheFresh(cache, now, ttlMs = MODELS_CACHE_TTL_MS) {
...
function writeModelsCache(path, models, now) {
  const cache = {
...
async function discoverModels(opts = {}) {
  ...
  if (cached && cacheFresh(cached, now, opts.ttlMs))
    return cached.models;
  const bin = opts.bin ?? resolveAgyBin(opts.env ?? process.env);
  let fetched = [];
  try {
    fetched = opts.list ? await opts.list(bin) : await listAgyModels({ bin, timeoutMs: opts.timeoutMs ?? DEFAULT_MODELS_TIMEOUT_MS });
  } catch {
    fetched = [];
  }
  if (fetched.length > 0) {

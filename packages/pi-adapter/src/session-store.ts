/**
 * Session-map store (spec R5): pi session key (`options.sessionId ?? cwd`)
 * → agy conversation id in `<stateDir>/agy-bridge/pi-sessions.json`. Every
 * read-modify-write cycle is serialized at three levels:
 *
 * 1. In-process mutexes — a global chain (file I/O) plus per-key keyed
 *    chains (same-session turns).
 * 2. A cross-process lockfile — each pi instance is a SEPARATE OS process,
 *    so mutexes alone still let two read-modify-write cycles interleave
 *    (A reads, B reads, A writes, B writes → A's binding is lost). Before
 *    touching the file, the store takes `<state-file>.lock` via an
 *    exclusive create (`openSync(lockPath, "wx")`, i.e. O_EXCL) and
 ...
 *
 * Divergence baseline: each entry optionally carries `hashes` — the ordered
 * per-message hashes of the forwarded prompt array AS SENT for that
 * conversation (engine messages.messageHashes). Entries written without
 ...
 * persisted rows survive every pi lifecycle event because each turn
 * re-reads its own row by key (`options.sessionId ?? cwd` supplied per
 * turn). Only the 30d prune removes rows.
 */
import {
	closeSync,
	mkdirSync,
...
export interface SessionEntry {
	...
	/** Divergence baseline: ordered hashes of the forwarded prompt array. Absent on hash-less entries (unknown baseline). */
	hashes?: string[];
}
...
export interface SessionStore {
	...
	/** Full entry including the divergence baseline; undefined when unbound. */
...
/**
 * Open the pi session store at `<stateDir>/pi-sessions.json` from the
 * resolved adapter config (R5: the config's concrete absolute stateDir
 * decides the location; no path is re-derived here).
 */
export function openPiSessionStore(config: { stateDir: string }, options: SessionStoreOptions = {}): SessionStore {
	return openSessionStore(join(config.stateDir, "pi-sessions.json"), options);
}
